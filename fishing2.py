...
def CheckIfHit():
    ...
    top_cut = int(0.5 * height)
    bottom_cut = height - int(0.37 * height)  # Cut 30% from the bottom
    left_cut = int(0.37 * width)
    right_cut = width - int(0.37 * width)
    screenshot = screenshot[top_cut:bottom_cut, left_cut:right_cut]  # Directly assign the cropped image to screenshot

    template_path = 'Situations/For.png'  # Replace with your template's path
    ...
    if max_val > 0.91:  # Define a threshold value that determines a successful match
        return 'hit'
...
def CheckIfRestart():
    ...
    top_cut = int(0.5 * height)
    bottom_cut = height - int(0.37 * height)  # Cut 30% from the bottom
    left_cut = int(0.30 * width)
    right_cut = width - int(0.4 * width)
    screenshot = screenshot[top_cut:bottom_cut, left_cut:right_cut]  # Directly assign the cropped image to screenshot

    template_paths = ['Situations/HitB2.png', 'Situations/caught.png', 'Situations/lost_Bait.png',
                      'Situations/lost_Hook.png','Situations/obtained.png']
    max_vals = []
    ...
    if best_template_path in ['Situations/HitB2.png'] :
        return 'BottomHit'
    elif best_template_path in ['Situations/caught.png','Situations/obtained.png'] :
        return 'caught'
...
def UltimateEye():
    ...
    template_paths = ['Rods/DarkCurveSmall.png', 'Rods/curvedLightSmall.png', 'Rods/DarkCurvedSmall2.png', 'Rods/moonStraightSmall.png',
                      'Rods/staightLightSmall.png', 'Rods/DarkStraightRodSmall.png', 'Rods/LightStraight.png','Rods/CurvedHalfDark.png']
    max_vals = []
    ...
    if best_template_path in ['Rods/DarkStraightRodSmall.png', 'Rods/staightLightSmall.png', 'Rods/LightStraight.png', 'Rods/moonStraightSmall.png']:
        return 'straight'
    if best_template_path in ['Rods/DarkCurveSmall.png', 'Rods/curvedLightSmall.png', 'Rods/DarkCurvedSmall2.png', 'Rods/CurvedHalfDark.png'] and np.argmax(max_vals)>=0.91:
        return 'bite'
...
def ThrowGarbage():
    ...
    for image in Garbage:
        if CheckIfHit() == "hit":
            notification.notify(
                title='Bot is taking damage',
                message='Bot is under attack',
                app_name='Bot')
    ...
    while CheckIfEmpty()=="full":
        ...
        if Eye=='bite':
            ...
            if UltimateEye()=='bite':
                ...
                #if NextMove=='LostBait' or NextMove=='LostHook':
                    #print(NextMove)
                    #return
                time.sleep(1)
                print("end")
...
def StartBot():
    while keyboard.is_pressed("q")==False:
     ...
     #   move()
        ...
        ThrowFishingRod()
        time.sleep(1)
        Checker=CheckIfRestart()
        print(Checker)
        if Checker == "BottomHit":
            notification.notify(
                title='Bottom hit',
                message='You cannot catch anything',
                app_name='Bot'
            )
            sys.exit()
        Fishing()
